...
$resetTitle = Yii::t('app', 'Reset');
$submitTitle = Yii::t('app', 'Save');
...
<?php if (!$node->isNewRecord || !empty($parentKey)): ?>
    ...
    ?>
    <?php if ($isAdmin): ?>
        <h4><?= Yii::t('app', 'Admin Settings') ?></h4>
            ...
            <?= $form->field($node, 'url_type')->dropDownList([
                ...
                'page'=>'Sahifalar',
                   ...
                   var t =  $(this).val()
                    ...
                    else if (t === "category" || t === "page" || t == "catalog" || t == "sub_catalog" || t == "c-action"){
                        $.ajax({
                            url: menu_url,
                            data: {type:t},
                            type: "get",
            ...
            <div class="form-group highlight-addon field-tree-url_value">
                ...
                <div id="menu-value">
                    ...
                    if ($node->url_type == 'category')
                        echo $form->field($node, 'url_value')->dropdownList(ArrayHelper::map(\backend\modules\postmanager\models\Post::find()->all(), 'slug', 'title' ))->label(false);
                    elseif($node->url_type == 'page')
                        echo $form->field($node, 'url_value')->dropdownList(ArrayHelper::map(\backend\modules\pagemanager\models\Page::find()->all(), 'identifikator', 'title' ))->label(false);
